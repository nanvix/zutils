...
from nanvix_zutil.exitcodes import EXIT_GENERAL_ERROR, EXIT_MISSING_DEP
...
class Sysroot:
    ...
    def from_local(local_path: Path, config: Config | None = None) -> "Sysroot":
        ...
        try:
            ...
            log.fatal(
                f"Failed to create symlink from {dest} to {resolved}: {e}",
                code=EXIT_MISSING_DEP,
                hint="Check permissions and try again. If you're on Windows, ensure dev mode is enabled.",
            )
    ...
    def download(
        ...
        if sysroot_dir.is_symlink():
            log.warning(
                f"Sysroot exists at {sysroot_dir}, but is a symlink. Unlinking it and continuing."
            )
            sysroot_dir.unlink()
        elif sysroot_dir.is_file():
            log.fatal(
                f"Sysroot exists at {sysroot_dir}, but is not a directory.",
                code=EXIT_GENERAL_ERROR,
                hint="Remove the blocking sysroot file.",
            )
        ...
        if config is not None:
            ...
            config.save()
        return Sysroot(tag=resolved_tag)
